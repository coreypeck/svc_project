...
router.post('/federal/:id', function(req, res) {
    ...
    var iLike = " iLIKE ";
    var is = " is ";
    var checkFirstTimer = " AND ((victim_prior_contact is false AND (victim_prior_oct is null OR victim_prior_oct is true OR victim_prior_oct is false)) OR (victim_prior_contact is true AND victim_prior_oct is true)) ";
    ...
    pool.connect(function(err, client, done) {
        ...
        if (text == "TOTAL") {
            ...
            checkFirstTimer = " ((victim_prior_contact is false AND (victim_prior_oct is null OR victim_prior_oct is true OR victim_prior_oct is false)) OR (victim_prior_contact is true AND victim_prior_oct is true)) ";
...
router.post('/county/:id', function(req, res) {
    ...
    pool.connect(function(err, client, done) {
        ...
        //     // console.log('victim age unknown:', query);
            ...
            query = selectDistinct + hennepin + checkFirstTimer + greaterThanOrEqual + "'" + dateStart + "'" + lessThan + "'" + dateEnd + "AND service_location IS NOT NULL'";
...
//special query to pull location names;
